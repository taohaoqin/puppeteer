...
(async () => {
     let fileName = 'template.js'
     let section = {from: 1, to: 3}//从1开始 表示打印第几组到第几组数组
    ...
    const obj = new Param(JSON.parse(params.content))
    obj.section = section ? Object.values(section) : []
    obj.stopTime = params.stopTime ? params.stopTime : 200
    let duration = (obj.dataOri[0].length - 2) / obj.second * 1000
    obj.duration = duration
    const browser = await puppeteer.launch({
        slowMo: 100,
        headless: true,
        defaultViewport: {width: obj.proportion === '9:16' ? 607 : 1920, height: 1080},
    ...
    await page.evaluate((obj) => {
      ...
      colorObj = obj.colorObj
      flagList = obj.flagList
      list = obj.section
      dateIndex = obj.section.length > 1 ? obj.section[0] : 1
      stopTime = obj.stopTime
      num = parseInt(25 / (1000 / duration)) - 1
      setColor()
      init()
      nextRender()
    }, obj)
    const res = await page.evaluate(() => {
      return dataOri[0].length - 2
    })
    console.log(res) //计算数据长度
    //判断是否有区间参数
    let a 
    if(obj.section.length > 0){
      if(obj.section[1] <= res){
        a = obj.section[1] - obj.section[0]
      } else {
        a = res + 1 - obj.section[0]
      }
    } else {
      a = res
    }
    if(!(obj.section[0] && obj.section[0] > 1)){
      await page.screenshot({path: `images/data0.png`}) //第一张
    }
    for(let i = 0; i < (a) * 25; i++){
        const reslute  = await page.evaluate(() => {
            return nextRender()
        })
        if(reslute){
            await page.screenshot({path: `images/data${i+1}.png`}) // 1 - 100张
        }
    }
    await browser.close()
    console.log('结束')
