...
var frame = 0, // is an animation frame pending?
    ...
    setFrame = function(f) { setTimeout(f, 40) };

function now() { //now()为计时器开始后的时间
  // console.log(clockNow, clock.now(), clockSkew)
...
function timer(callback, delay, time) {
  ...
  // console.log(delay, time) //time 为计时器开始后的时间
  // console.log(t)  //构造函数new Timer对象 原型指向timer的原型
  return t;
}

function timerFlush() {
  // console.log(now())now()为计时器开始后的时间
...
function timeout$1(callback, delay, time) {
  ...
  t.restart(elapsed => {
    t.stop();
    // console.log(elapsed, delay, time) // 打出来的都是 0 0  
...
function create$1(node, id, self) {
  ...
  function start(elapsed) {
    ...
    for (i in schedules) {
      ...
      // console.log(o.state, STARTED, 3333)
  ...
  function tick(elapsed) {
    let st = 25 - parseInt(stopTime / 40)
    // console.log(elapsed)
    // console.log(num)
    // console.log(cishu)
    // console.log(elapsed, '1111')
    //elapsed 计时时间  当elapsed  > duration过渡时间的时候 停止
    var t = elapsed < 40 ? self.ease.call(null, elapsed / 40) : (self.timer.restart(stop), self.state = ENDING, 1),
        i = -1,
        n = tween.length;
    // console.log(t, 't')
    // console.log(n, 'n')
    
    let b = t * ((1 / st) * (1000 / duration))  * num > 1 ? 1 : t * ((1 / st) * (1000 / duration)) * num //用来判断中途停留几帧 
    while (++i < n) { //对每一个标签变化
      tween[i].call(node, b);
      // tween[i].call(node, t); // t为计时器启动时的时间now 与 过渡时间之比 从而控制变化过程 如果t/2 就会变化到一半
    }

    // Dispatch the end event.
    if (self.state === ENDING) {
      // console.log('ENDING')
      // console.log(node, 'node') //按g标签  rect标签  text标签打印
      // console.log(node.__data__, 'node.__data__')
...
function interrupt(node, name) {
  // console.log(node, name,'interrupt')
  var schedules = node.__transition,
      schedule,
      active,
      empty = true,
